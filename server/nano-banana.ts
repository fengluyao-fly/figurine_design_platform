...
export async function generateThreeViews(
  ...
  if (imageUrl) {
    ...
    if (prompt && prompt.trim()) {
      ...
      threeViewPrompt = `Create a product design sheet with four orthographic views (front, left side, back, right side) of this subject. CRITICAL VIEW REQUIREMENTS: 1) FRONT VIEW (leftmost): face/front side visible, looking straight ahead. 2) LEFT SIDE VIEW (second): left profile, 90° rotation from front. 3) BACK VIEW (third): back of head/rear side visible, opposite of front. 4) RIGHT SIDE VIEW (rightmost): right profile, 90° rotation from front. CRITICAL CROPPING: Match the EXACT CROPPING of the reference image. If the reference shows only head and upper body (no legs, no feet), then ALL FOUR VIEWS must show only head and upper body - DO NOT add legs or feet. If the reference shows full body, then show full body. NEVER extrapolate or invent body parts not visible in the reference. CRITICAL ORIENTATION RULE: ALL FOUR VIEWS MUST MAINTAIN IDENTICAL VERTICAL ALIGNMENT - head always at the TOP of the frame, feet/torso always at the BOTTOM of the frame. The views differ ONLY in horizontal rotation angle (0°, 90°, 180°, 270°), NOT in vertical orientation. NEVER rotate the subject upside-down or sideways vertically. Preserve exact style, colors, clothing, and pose. Layout: arranged horizontally from left to right. White background, clean separation.`;
    }
  } else {
