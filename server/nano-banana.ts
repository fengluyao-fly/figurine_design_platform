...
export async function generateThreeViews(
  ...
  if (imageUrl) {
    ...
    if (prompt && prompt.trim()) {
      ...
      threeViewPrompt = `Product design sheet showing four orthographic views of: ${prompt}. CRITICAL VIEW REQUIREMENTS: 1) FRONT VIEW (leftmost): face/front side visible, looking straight ahead. 2) LEFT SIDE VIEW (second): left profile, 90° rotation from front. 3) BACK VIEW (third): back of head/rear side visible, opposite of front. 4) RIGHT SIDE VIEW (rightmost): right profile, 90° rotation from front. Layout: arranged horizontally from left to right. Show the COMPLETE FULL BODY from head to toe, do not crop any part. CRITICAL ORIENTATION RULE: ALL FOUR VIEWS MUST MAINTAIN IDENTICAL VERTICAL ALIGNMENT - head always at the TOP of the frame, feet always at the BOTTOM of the frame. The views differ ONLY in horizontal rotation angle (0°, 90°, 180°, 270°), NOT in vertical orientation. NEVER rotate the subject upside-down or sideways vertically. White background, professional product photography, studio lighting, clean separation between views.`;
    } else {
      // User provided only image, no text: strictly preserve original style and composition
      threeViewPrompt = `Create a product design sheet with four orthographic views (front, left side, back, right side) of this subject. CRITICAL VIEW REQUIREMENTS: 1) FRONT VIEW (leftmost): face/front side visible, looking straight ahead. 2) LEFT SIDE VIEW (second): left profile, 90° rotation from front. 3) BACK VIEW (third): back of head/rear side visible, opposite of front. 4) RIGHT SIDE VIEW (rightmost): right profile, 90° rotation from front. ABSOLUTELY CRITICAL CROPPING RULES - MUST FOLLOW EXACTLY: 1) Match the EXACT vertical cropping of the reference image. 2) If reference shows ONLY HEAD AND UPPER BODY (shoulders, chest) with NO LEGS visible, then ALL FOUR VIEWS MUST show ONLY HEAD AND UPPER BODY - ABSOLUTELY FORBIDDEN to add legs, feet, hips, or lower body. 3) If reference shows full body from head to feet, then show full body. 4) NEVER extrapolate, invent, or imagine body parts not visible in the reference. 5) The bottom edge of all four views MUST match the bottom edge of the reference image exactly. CRITICAL ORIENTATION RULE: ALL FOUR VIEWS MUST MAINTAIN IDENTICAL VERTICAL ALIGNMENT - head always at the TOP of the frame, torso/feet always at the BOTTOM of the frame. The views differ ONLY in horizontal rotation angle (0°, 90°, 180°, 270°), NOT in vertical orientation. NEVER rotate the subject upside-down or sideways vertically. Preserve exact style, colors, clothing, pose, and proportions. Layout: arranged horizontally from left to right with equal spacing. White background, clean separation between views.`;
    }
  } else {
    // Text-only input (no image)
    threeViewPrompt = `Product design sheet, ${prompt}, four orthographic views. CRITICAL VIEW REQUIREMENTS: 1) FRONT VIEW (leftmost): face/front side visible, looking straight ahead. 2) LEFT SIDE VIEW (second): left profile, 90° rotation from front. 3) BACK VIEW (third): back of head/rear side visible, opposite of front. 4) RIGHT SIDE VIEW (rightmost): right profile, 90° rotation from front. Layout: arranged horizontally from left to right. Show the COMPLETE FULL BODY from head to toe. CRITICAL ORIENTATION RULE: ALL FOUR VIEWS MUST MAINTAIN IDENTICAL VERTICAL ALIGNMENT - head always at the TOP of the frame, feet always at the BOTTOM of the frame. The views differ ONLY in horizontal rotation angle (0°, 90°, 180°, 270°), NOT in vertical orientation. NEVER rotate the subject upside-down or sideways vertically. White background, professional product photography, studio lighting, technical drawing style, clean layout.`;
  }
